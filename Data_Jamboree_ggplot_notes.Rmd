...
#Begin Live Coding: 
Alison:
-Reminder to download R, RStudio, and packages
...
Now, R has been bugging us about the binwidth of our histogram by telling us "## `stat_bin()` using `bins = 30`. Pick better value with `binwidth`." This just tells us that this is a default that ggplot chose for us. Defaults are in most commands for ggplot. It chose these because it believes it makes the most sense. It wants to make this whole plotting this really easy for you. You can always look up the documentation to see what the defaults are and change them to your liking. 

For now, we will change the binwidth to 50. Feel free to play around with this. You can change this to any number.  
...
Say that we want our plot to give us the kernal density instead. 
...
Remeber from before colour = the outline. So our density plots aren't filled yet. 
...
We'll start with the same basic structure from above, 

"ggplot(data, aesthetics (x and y axis)) + add our layer on top to get our dotplot", but this time we will do geom_ point instead of a geom_ histogram or a geom_ density. 
...
As you can see, there are way too many points to understand what this is trying to say. We can add an alpha in the geom layer to make our points more transparent.
...
Alright, so we can see all of our points now, but the start to get a little mixed up with each other. We can change the width and the height of our jitter. First, we should think a bit about what jitter does? It spaces things out and can introduce some noise to our plot. Since our x-axis is a categorical variable we won't be adding too much noise on our x axis, but our y-axis is continuous so jitter there will really mess up our graph. 
...
Now we should can add a title on our x-axis and we have a readable plot!
...
Say we want to look at this same type of plot but as a beeswarm. We copy and paste our same code from the dot plot we made above, but we replace geom_ jitter with geom_ beeswarm.
...
Once again, our points seem to overlap each other a bit, so let's add some alpha again. We'll add in in our beeswarm layer. 
...
ggplot doesn't just make plots, you can also add statistics on top of your plots. Say we want to see where the mean and the 95% confidence interval are in our plots. We will add a new layer for each by using stat_summary. Now the language here gets a bit more complicated but what we are telling R to do is to give us the mean, what type of geom it should be, and what color it should be. 

Play around with your colors too!

**ALISON: Hmisc package required for this ci function** 
```{r}
ggplot(heart, aes(x = gender, y = BPXSAR)) +
  geom_beeswarm(alpha = .2) +
...
Ok, say instead of adding statistics on our plot, we want another plot type in there. We can also add that as a layer by using the + geom syntax. We'll add a boxplot in our beeswarm plot to give ourselves a better picture of our beeswarm. 
...
Alright, now let's say that we want to do a violin plot instead of our boxplot. Easy, just replace geom_ boxplot with geom_ violin.
...
Just like before, we can add our statistics using stat_summary. Here we can add our mean and median. 
...
You say you want a violin plot and a boxplot. Just add it as a layer. You can change the boxplot's size by specifying it's width. 
